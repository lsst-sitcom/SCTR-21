...
\multicolumn{2}{p{10.9cm}}{\textbf{ M2 Hexapod Re-verification and Integration Testing }} & Done \\\hline
...
\tightlist
\item
  During the February 2020 verification, the M2 hexapod software was
  still under development at its various levels. (see
  https://jira.lsstcorp.org/secure/Tests.jspa\#/testCase/LVV-T1804)
\item
  During the February 2020 verification, the M2 hexapod presented random
  faults in Drive 0 (see https://jira.lsstcorp.org/browse/FRACAS-53)
\item
  The M2 hexapod needed to be installed at the TMA. Therefore some of
  the tests could not be executed.
\item
  Some hardware tests on the accuracy using the laser tracker failed.
  (see https://jira.lsstcorp.org/secure/Tests.jspa\#/testCase/LVV-T1800)
\item
  Some of the CSC tests failed (see
  https://jira.lsstcorp.org/secure/Tests.jspa\#/testCase/LVV-T1802)
\item
\end{itemize}
...
\tightlist
\item
  The low-level controller, the EUI, and the CSC should be updated to
  the latest versions.
\item
  Clean the actuator similar to the Camera hexapod actuators and
  reassess the MOOG tests 3.5.12 and 3.5.24 to see if the requirements
  are met afterward.
\item
  The cables for the M2 hexapod should be updated similar to the camera
  hexapod and the 32 MOOG test moves (3.5.12) and the endurance test
  should be executed.
\item
  FRACAS-53 (https://jira.lsstcorp.org/browse/FRACAS-53) should be
  closed out.
\item
  Enough time should be allocated to test the EUI commands and the rest
  of the hardware tests.
\item
  The software tests for the EUI should be executed when the state
  machine is updated to standbyState-entry state machine. The test case
  should be updated to account for state machine tests.
\item
  The software tests for the CSC should be executed again to account for
  the changes done to the CSC.
\item
  The configuration part of the test cases should be updated to account
  for the change in accordance with \citeds{LSE-209}.
\item
  The configuration handling should be tested.
\end{itemize}
...
\begin{tabular}{p{2cm}p{14cm}}
\toprule
Step 10 & Step Execution Status: \textbf{ Pass w/ Deviation } \\ \hline
\end{tabular}
 Description \\
{\footnotesize
...
repeatability testsâ\euro{}¦not done, no time~
...
\tightlist
\item
  Done by Felipe when changing the limit switch logic to normally closed
\end{itemize}
